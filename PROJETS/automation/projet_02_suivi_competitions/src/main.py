...
from models import Competition, Statut
from services.gestionnaire_competitions import GestionnaireCompetitions
...
class Projet02SuiviCompetitions:
    ...
    def __init__(self):
        ...
        self.service = GestionnaireCompetitions()
    ...
    def _setup_logging(self):
        ...
        return logging.getLogger("projet_02_suivi_competitions")
    
    def _afficher_banniere(self):
        """Affiche la bannière."""
        print(f"\n{Colors.BOLD}{Colors.BLUE}")
        print(f"╔══════════════════════════════════════════════════╗")
        print(f"║  SUIVI DES COMPÉTITIONS               ║")
        print(f"╚══════════════════════════════════════════════════╝")
...
def main():
    """Point d'entrée."""
    parser = argparse.ArgumentParser(description="Suivi des Compétitions")
    parser.add_argument("--version", action="version", version="%(prog)s 1.0.0")
    
    args = parser.parse_args()
    
    app = Projet02SuiviCompetitions()
    try:
        app.run()
