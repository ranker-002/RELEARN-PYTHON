...
Système ETL (Extract, Transform, Load) pour flux de données

---

## 🎯 Objectif du Projet

Créer un pipeline de données configurable pour automatiser les traitements ETL
...
1. **Configuration visuelle des pipelines**
2. **Extraction depuis CSV, JSON, API, bases de données**
3. **Transformations (filtrage, mapping, agrégation)**
4. **Chargement vers différentes destinations**
5. **Exécution planifiée ou manuelle**
6. **Monitoring et logs des exécutions**
...
@dataclass
class Pipeline:
    id, nom, description, etapes_ids, statut
...
@dataclass
class Etape:
    id, pipeline_id, type, configuration, ordre
...
@dataclass
class Source:
    id, nom, type, parametres_connexion
...
### Orchestrateur
**Description :** Orchestre l'exécution des pipelines

**Fichier :** `src/services/orchestrateur.py`
...
### Transformateur
**Description :** Applique les transformations

**Fichier :** `src/services/transformateur.py`
