...
Outil d'analyse statistique et visualisation de datasets

---

## 🎯 Objectif du Projet

Créer une application pour explorer et analyser des jeux de données
...
1. **Import de données CSV, Excel, JSON**
2. **Aperçu et profilage des données (types, valeurs manquantes)**
3. **Statistiques descriptives (moyenne, médiane, quartiles)**
4. **Détection d'anomalies et outliers**
5. **Visualisations interactives (histogrammes, boxplots, scatter)**
6. **Génération de rapports d'analyse**
...
@dataclass
class JeuDeDonnees:
    id, nom, chemin, format, nb_lignes, nb_colonnes
...
@dataclass
class Analyse:
    id, dataset_id, type, resultats, date_analyse
...
@dataclass
class Visualisation:
    id, analyse_id, type_graphique, parametres, chemin_image
...
@dataclass
class Rapport:
    id, dataset_id, contenu, date_generation
...
### Explorateur
**Description :** Explore les datasets

**Fichier :** `src/services/explorateur.py`
...
**Fichier :** `src/services/statisticien.py`
