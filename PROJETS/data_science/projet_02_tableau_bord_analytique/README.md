...
Dashboard interactif pour visualiser des métriques et KPIs

---

## 🎯 Objectif du Projet

Créer un tableau de bord avec widgets personnalisables pour le suivi de métriques
...
1. **Création de tableaux de bord personnalisables**
2. **Widgets variés (graphiques, indicateurs, tableaux)**
3. **Connexion à des sources de données multiples**
4. **Mise à jour en temps réel ou planifiée**
5. **Alertes sur seuils dépassés**
6. **Partage et export des dashboards**
...
@dataclass
class TableauBord:
    id, nom, description, widgets_ids, date_modification
...
@dataclass
class Widget:
    id, dashboard_id, type, titre, configuration, position
...
@dataclass
class Metrique:
    id, widget_id, nom, valeur, unite, seuil_alerte
...
@dataclass
class Alerte:
    id, metrique_id, condition, valeur_seuil, actif
...
### Gestionnaire Tableaux
**Description :** Gère les dashboards

**Fichier :** `src/services/gestionnaire_tableaux.py`
...
### Agregateur
**Description :** Agrège les données pour les widgets

**Fichier :** `src/services/agregateur.py`
