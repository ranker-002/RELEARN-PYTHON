...
Système de classification d'images avec apprentissage profond

---

## 🎯 Objectif du Projet

Créer un système pour classifier des images dans différentes catégories
...
2. **Augmentation de données (rotation, zoom, flip)**
3. **Création de modèles de classification**
4. **Entraînement avec validation croisée**
5. **Évaluation des performances (accuracy, precision, recall)**
...
@dataclass
class Modele:
    id, nom, architecture, classes, date_entrainement
...
@dataclass
class Image:
    id, chemin, classe_reelle, classe_predite, confiance
...
@dataclass
class Prediction:
    id, modele_id, image_id, classe_predite, confiance, date
...
### Classificateur
**Description :** Classifie les images

**Fichier :** `src/services/classificateur.py`
...
### Preprocesseur
**Description :** Prétraite les images

**Fichier :** `src/services/preprocesseur.py`
