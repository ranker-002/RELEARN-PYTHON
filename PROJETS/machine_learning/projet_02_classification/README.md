...
Classification supervisée avec évaluation des performances

---

## 🎯 Objectif du Projet

Créer un système de classification avec métriques détaillées
...
1. **Préparation des données de classification**
2. **Choix d'algorithmes (KNN, SVM, Decision Tree, etc.)**
3. **Entraînement avec grid search pour hyperparamètres**
...
class Classificateur:
    id, nom, algorithme, parametres, date_entrainement
...
@dataclass
class Donnees:
    id, nom, X_train, X_test, y_train, y_test
...
@dataclass
class Classe:
    id, classificateur_id, nom, precision, rappel, f1_score
...
@dataclass
class Evaluation:
    id, classificateur_id, accuracy, matrice_confusion, date
...
### Entraineur
**Description :** Entraîne les classificateurs
...
### Evaluateur
**Description :** Calcule les métriques

**Fichier :** `src/services/evaluateur.py`
